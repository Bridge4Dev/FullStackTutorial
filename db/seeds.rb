...
#Cache the number of states so we don't have to query each time
...
Customer.all.pluck(:id).each do |customer_id|
  ...
  #Create a random number of shipping addresses, making sure at least 1
  num_shipping_addresses = rand(4) + 1

  num_shipping_addresses.times do |i|
    #Create the shipping address, setting first as primary
